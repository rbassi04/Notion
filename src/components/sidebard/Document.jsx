...
export const Document = ({currDocument, setWorkshop, workshop_id, admin}) => {
  ...
  async function deleteDocument() {
    ...
    const {error} = await supabase.from("documents").delete().in("id", toDeleteIds)
  ...
  async function shareDocument() {
    ...
    setWorkshop(workshop => {
      ...
      const newDocuments = currWorkshop['documents'].map(doc => 
        doc.id !== currDocument.id ? doc : {...doc, shared_id: new_shared_id}
      )
  ...
  return (
    <div>
      ...
      <div className='flex gap-1 px-1 mb-1 rounded-md font-inter text-[#e4e4e4] '>
        {
          isLeafDocument ? (
            <button onClick={toggleDisplayChildren} className='text-xs hover:font-black hover:cursor-pointer hover:scale-125 transition'>
              {displayChildren ? "▼" : <span className='text-[9px]'>▶</span>}
            </button>
          ) : <p className=''>{"-"}</p>
        }
        <div className='w-full flex justify-between'>
          <Link to={`/document/${currDocument.id}`} className='text-ellipsis truncate w-full pl-[6px] mr-2 rounded-sm hover:cursor-pointer hover:bg-[#272727]'>
            {
              currDocument.name
            }
          </Link>
          <div ref={menuRef}  className='flex relative gap-[1px] items-center'>
            <button 
            ...
            <button onClick={() => {setNewDocumentPopup(true)}} className='hover:cursor-pointer opacity-20 scale-75 transition hover:scale-100 hover:opacity-100'>➕</button>
            {
              showMenu &&
                <div className='flex flex-col justify-start gap-[2px] rounded-md absolute top-full right-0 z-10 bg-slate-800 px-1 py-3 '>
                  <button onClick={() => {setRenamePopup(true); setShowMenu(false)}} className='hover:cursor-pointer scale-75 transition hover:scale-85 hover:opacity-100 w-full text-left'>Rename</button>
                  <button onClick={deleteDocument} className='hover:cursor-pointer scale-75 transition hover:scale-85 hover:opacity-100 w-full text-left'>Delete</button>
                  <button onClick={shareDocument} className='hover:cursor-pointer scale-75 transition hover:scale-85 hover:opacity-100 w-full text-left'>{currDocument.shared_id ? "unShare" : "Share"}</button>
                </div>
            }
          </div>
        </div>
      ...
      <div className='pl-3 '>
        {
          displayChildren ? Object.keys(currDocument.children).map(child_id =>
            <Document 
              key={child_id} 
      ...
      {
        renamePopup && (
          <PopupRename prompt={"Rename your document:"} renameSubmit={renameSubmit} setRenamePopup={setRenamePopup} />
        )
      }
